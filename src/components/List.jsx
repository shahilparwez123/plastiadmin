...
const List= () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                const {data} = await axios.get('https://plastibackend.onrender.com/api/items');
                setItems(data);
            }
            catch(err){
    ...
    const handleDelete = async (itemId) => {
        if(!window.confirm('Are you want to delete this item?')) return;

        try{
            await axios.delete(`https://plastibackend.onrender.com/api/items/${itemId}`);
                                ...
                                {items.map(item => (
                                    <tr key={item._id} className={styles.tr}>
                                        ...
                                        <td className={styles.ratingCell}>
                                            <div className=' flex gap-1 '> {renderStars(item.rating)}
